...
const ListUsers: FC<ListProps> = ({ input }) => {
  ...
  const navigate = useNavigate();
    ...
    const addParticipantToAddedList = (participant) => {
    ...
    const removeParticipantFromAddedList = (participantId) => {
  ...
  const filterParticipants = (participants, input) => {
  ...
  //CREATION DES INVITATIONS
    const handleSubmit = async () => {
        const invitations = participantsAdded.map((participant) => ({
            holidayId: id,
    ...
    <>
      <div id="addedParticipant" className="mb-6">
          ...
          <ul className="flex w-full flex-wrap justify-center max-h-52 overflow-y-scroll">
              {participantsAdded.map((participant) => (
                  <li className="text-blue-800 hover:text-white border border-blue-700 hover:border-red-900 hover:bg-red-700 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-3 py-2 text-center mr-2 mb-2"
                      key={participant.id}
                      onClick={() => removeParticipantFromAddedList(participant.id)}>
                      {participant.firstName} {participant.lastName} <FontAwesomeIcon icon={faTimes} size="lg" className="ml-2.5" />
                  </li>
              ))}
          </ul>
      </div>
      ...
      <ul className=" max-h-52 overflow-y-scroll w-full mb-4 ">
        ...
        <div className="flex w-full justify-center">
            <button className="bg-blue-800 hover:bg-blue-700 text-white font-bold py-1 px-4 rounded-full" onClick={handleSubmit}>
                Ajouter
            </button>
        </div>
    </>
